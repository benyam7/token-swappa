...
export function QuickTokenSelector({
    ...
    return (
        <div
            className={`absolute -top-10 right-0 flex justify-end gap-1 z-10 p-1 ${
            ...
            {popularTokens.map((token, index) => {
                ...
                return (
                    <button
                        ...
                        className={`w-8 h-8 bg-white dark:bg-gray-800 rounded-full shadow-lg border-2 border-gray-100 dark:border-gray-700 hover:border-pink-300 hover:scale-110 transition-all duration-200 flex items-center justify-center group focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 ${animationClass}`}
                        style={{ animationDelay: `${delay}ms` }} // Apply dynamic delay
                        title={token.name}
                        aria-label={`Select ${token.name} (${token.symbol})`}
                    >
                        <div
                            className={`w-6 h-6 ${token.color} rounded-full flex items-center justify-center text-white text-xs font-bold group-hover:scale-110 transition-transform`}
                            aria-hidden="true"
                        >
                            {token.logo}
                        </div>
